...
redo:
    ...
    if (s->http_code == 401) {
        if ((cur_auth_type == HTTP_AUTH_NONE || s->auth_state.stale) &&
			...
			if (pfun_set_player_error_code) {
				pfun_set_player_error_code(h->interrupt_callback.opaque,ERROR_SOFT_PLAYER_HTTP_4XX);
			}
			goto fail;
        }
    ...
    if ((s->http_code == 301 || s->http_code == 302 ||
        ...
        if (redirects++ >= MAX_REDIRECTS){
        	//yanjia
        	if (pfun_set_player_error_code) {
        		pfun_set_player_error_code(h->interrupt_callback.opaque,ERROR_SOFT_PLAYER_HTTP_3XX);
        	}
            return AVERROR(EIO);
        }
