...
void ff_hevc_deblocking_boundary_strengths_onepu(HEVCContext *s, int x0, int y0,
                                           int log2_trafo_size)
{
    HEVCLocalContext *lc = s->HEVClc;
    ...
    int i, j, bs;
	CLK_DEF(dblk_strength1);
	
	CLK_T0(dblk_strength1);
    ...
    if (log2_trafo_size > log2_min_pu_size && !is_intra) {
        ...
        for (j = 8; j < (1 << log2_trafo_size); j += 8) {
            ...
            for (i = 0; i < (1 << log2_trafo_size); i += 4) {
                ...
                if (bs != 0|| s->horizontal_bs[((x0 + i) + (y0 + j) * s->bs_width) >> 2] != 0){
                   LOGI_ERR("The prev boundary strength is %d next bs is %d\n", s->horizontal_bs[((x0 + i) + (y0 + j) * s->bs_width) >> 2], bs);
                }				
                s->horizontal_bs[((x0 + i) + (y0 + j) * s->bs_width) >> 2] = bs;
            }
        }
        ...
        for (j = 0; j < (1 << log2_trafo_size); j += 4) {
            ...
            for (i = 8; i < (1 << log2_trafo_size); i += 8) {
				...
				if (bs != 0|| s->vertical_bs[((x0 + i) + (y0 + j) * s->bs_width) >> 2] != 0){
                   LOGI_ERR("The prev boundary strength is %d next bs is %d\n", s->horizontal_bs[((x0 + i) + (y0 + j) * s->bs_width) >> 2], bs);
                }	
                                           ...
                                           int log2_trafo_size)
{
    ...
    int boundary_upper, boundary_left;
    int i, j, bs;
	CLK_DEF(dblk_strength);
	
	CLK_T0(dblk_strength);
...
void ff_hevc_hls_filter(HEVCContext *s, int x, int y, int ctb_size)
{
	...
	CLK_DEF(tff_hevc_hls_filter);
	CLK_DEF(tff_deblocking);

	CLK_T0(tff_hevc_hls_filter);
	CLK_T0(tff_deblocking);	
    deblocking_filter_CTB(s, x, y);
	CLK_T1(tff_deblocking);	
