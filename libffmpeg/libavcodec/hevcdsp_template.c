...
{
    //CLK_DEF(n_tput_hevc_pel_pixels);
    //CLK_T0(n_tput_hevc_pel_pixels);  
    int x, y;
    pixel *src          = (pixel *)_src;
                                          ...
                                          int height, intptr_t mx, intptr_t my, int width)
{
	...
	//CLK_T1(n_tput_hevc_pel_uni_pixels);  	
}
...
static void FUNC(put_hevc_pel_bi_pixels)(uint8_t *_dst, ptrdiff_t _dststride, uint8_t *_src, ptrdiff_t _srcstride,
                                         ...
                                         int height, intptr_t mx, intptr_t my, int width)
{
    //CLK_DEF(n_tput_hevc_pel_bi_pixels);
    //CLK_T0(n_tput_hevc_pel_bi_pixels);  
    ...
    //CLK_T1(n_tput_hevc_pel_bi_pixels);  	
}
                                  ...
                                  int height, intptr_t mx, intptr_t my, int width)
{
	...
	//CLK_DEF(n_tput_hevc_qpel_h)
	//CLK_T0(n_tput_hevc_qpel_h)		
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++)
                                  ...
                                  int height, intptr_t mx, intptr_t my, int width)
{
	...
	//CLK_DEF(n_tput_hevc_qpel_v)
	//CLK_T0(n_tput_hevc_qpel_v)		
    for (y = 0; y < height; y++)  {
        for (x = 0; x < width; x++)
	...
	//CLK_T1(n_tput_hevc_qpel_v)	
}

#ifdef QPEL_HV_SHORT_BW
                                   ...
                                   intptr_t my, int width)
{
	...
	//CLK_DEF(n_tput_hevc_qpel_hv)
	//CLK_T0(n_tput_hevc_qpel_hv)		

    src   -= QPEL_EXTRA_BEFORE * srcstride;
    filter = ff_hevc_qpel_filters[mx - 1];
    for (y = 0; y < height + QPEL_EXTRA; y++) {
        for (x = 0; x < width; x++)
            tmp[x] = av_clip_pixel( (QPEL_FILTER(src, 1) + 32) >> 6) ;
        src += srcstride;
        tmp += MAX_PB_SIZE;
    }
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++)
            dst[x] = QPEL_FILTER(tmp, MAX_PB_SIZE);
        tmp += MAX_PB_SIZE;
        dst += MAX_PB_SIZE;
    }
	//CLK_T1(n_tput_hevc_qpel_hv)
}
                                      ...
                                      int height, intptr_t mx, intptr_t my, int width)
{
    ...
    const int8_t *filter    = ff_hevc_qpel_filters[mx - 1];
    int shift = 14 - BIT_DEPTH;
	//CLK_DEF(n_tput_hevc_qpel_uni_h);
	//CLK_T0(n_tput_hevc_qpel_uni_h);				
                                     ...
                                     int height, intptr_t mx, intptr_t my, int width)
{
    ...
    const int8_t *filter    = ff_hevc_qpel_filters[mx - 1];
    //CLK_DEF(n_tput_hevc_qpel_bi_h);
    //CLK_T0(n_tput_hevc_qpel_bi_h);	
    int shift = 14  + 1 - BIT_DEPTH;
#if BIT_DEPTH < 14
    int offset = 1 << (shift - 1);
#else
                                     ...
                                     int height, intptr_t mx, intptr_t my, int width)
{
    ...
    const int8_t *filter    = ff_hevc_qpel_filters[my - 1];
    int shift = 14 - BIT_DEPTH;
	//CLK_DEF(n_tput_hevc_qpel_uni_v);
	//CLK_T0(n_tput_hevc_qpel_uni_v);			
                                     ...
                                     int height, intptr_t mx, intptr_t my, int width)
{
    ...
    const int8_t *filter    = ff_hevc_qpel_filters[my - 1];
	//CLK_DEF(n_tput_hevc_qpel_bi_v);
	//CLK_T0(n_tput_hevc_qpel_bi_v);	
	...
	//CLK_T1(n_tput_hevc_qpel_bi_v);
}

#ifdef QPEL_UNI_HV_SHORT_BW
                                       ...
                                       int height, intptr_t mx, intptr_t my, int width)
{
    ...
    uint8_t *tmp = tmp_array;
    int shift =  14 - BIT_DEPTH;
	//CLK_DEF(n_tput_hevc_qpel_uni_hv);
	//CLK_T0(n_tput_hevc_qpel_uni_hv);	
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++)
            dst[x] = av_clip_pixel(((QPEL_FILTER(tmp, MAX_PB_SIZE)) + offset) >> shift);
        tmp += MAX_PB_SIZE;
        dst += dststride;
    }
	//CLK_T1(n_tput_hevc_qpel_uni_hv);	
}
#else
static void FUNC(put_hevc_qpel_uni_hv)(uint8_t *_dst,  ptrdiff_t _dststride,
                                       uint8_t *_src, ptrdiff_t _srcstride,
                                      ...
                                      int height, intptr_t mx, intptr_t my, int width)
{
    ...
    int offset = 0;
#endif
	//CLK_DEF(n_tput_hevc_qpel_bi_hv);
	//CLK_T0(n_tput_hevc_qpel_bi_hv);
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++)
            dst[x] = av_clip_pixel(((QPEL_FILTER(tmp, MAX_PB_SIZE)) + src2[x] + offset) >> shift);
        tmp  += MAX_PB_SIZE;
        dst  += dststride;
        src2 += MAX_PB_SIZE;
    }
	//CLK_T1(n_tput_hevc_qpel_bi_hv);
}
#else
static void FUNC(put_hevc_qpel_bi_hv)(uint8_t *_dst, ptrdiff_t _dststride, uint8_t *_src, ptrdiff_t _srcstride,
                                      int16_t *src2,
                                        ...
                                        intptr_t mx, intptr_t my, int width)
{
    ...
    int offset = 0;
#endif
	LOGI_point(put_hevc_qpel_uni_w_h, "put_hevc_qpel_uni_w_h point %d %d %d", denom, wx, ox);
    ox = ox * (1 << (BIT_DEPTH - 8));
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++)
                                        ...
                                        intptr_t mx, intptr_t my, int width)
{
    ...
    int offset = 0;
#endif
	LOGI_point(put_hevc_qpel_uni_w_v, "put_hevc_qpel_uni_w_h point %d %d %d", denom, wx, ox);
                                         ...
                                         intptr_t mx, intptr_t my, int width)
{
    ...
    int offset = 0;
#endif
	LOGI_point(put_hevc_qpel_uni_w_hv, "put_hevc_qpel_uni_w_h point %d %d %d", denom, wx, ox);
                                  ...
                                  int height, intptr_t mx, intptr_t my, int width)
{
	...
	//CLK_DEF(n_put_hevc_epel_h)
	//CLK_T0(n_put_hevc_epel_h)
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++)
                                  ...
                                  int height, intptr_t mx, intptr_t my, int width)
{
	...
	//CLK_DEF(n_put_hevc_epel_v)
	//CLK_T0(n_put_hevc_epel_v)
	...
	//CLK_T1(n_put_hevc_epel_v)	
}


#ifdef EPEL_HV_SHORT_BW
                                   ...
                                   int height, intptr_t mx, intptr_t my, int width)
{
    ...
    uint8_t *tmp = tmp_array;
	CLK_DEF(n_put_hevc_epel_hv)
	CLK_T0(n_put_hevc_epel_hv)

    src -= EPEL_EXTRA_BEFORE * srcstride;

    for (y = 0; y < height + EPEL_EXTRA; y++) {
        for (x = 0; x < width; x++)
            tmp[x] = av_clip_pixel( ((EPEL_FILTER(src, 1) >> (BIT_DEPTH - 8)) + 32) >> 6);
        src += srcstride;
        tmp += MAX_PB_SIZE;
    }
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++)
            dst[x] = EPEL_FILTER(tmp, MAX_PB_SIZE);
        tmp += MAX_PB_SIZE;
        dst += MAX_PB_SIZE;
    }
	CLK_T1(n_put_hevc_epel_hv)
}
                                      ...
                                      int height, intptr_t mx, intptr_t my, int width)
{
    ...
    const int8_t *filter = ff_hevc_epel_filters[mx - 1];
    int shift = 14 - BIT_DEPTH;
	//CLK_DEF(n_tput_hevc_epel_uni_h);
	//CLK_T0(n_tput_hevc_epel_uni_h);	
#if BIT_DEPTH < 14
    int offset = 1 << (shift - 1);
#else
    int offset = 0;
                                     ...
                                     int height, intptr_t mx, intptr_t my, int width)
{
    ...
    const int8_t *filter = ff_hevc_epel_filters[mx - 1];
    int shift = 14 + 1 - BIT_DEPTH;
	CLK_DEF(n_tput_hevc_epel_bi_h);
	CLK_T0(n_tput_hevc_epel_bi_h);		
#if BIT_DEPTH < 14
    int offset = 1 << (shift - 1);
#else
    int offset = 0;
                                      ...
                                      int height, intptr_t mx, intptr_t my, int width)
{
    ...
    const int8_t *filter = ff_hevc_epel_filters[my - 1];
    int shift = 14 - BIT_DEPTH;
	//CLK_DEF(n_tput_hevc_epel_uni_v)
	//CLK_T0(n_tput_hevc_epel_uni_v)
#if BIT_DEPTH < 14
    int offset = 1 << (shift - 1);
#else
    int offset = 0;
                                     ...
                                     int height, intptr_t mx, intptr_t my, int width)
{
    ...
    int offset = 0;
#endif
	CLK_DEF(n_tput_hevc_epel_bi_v);
	CLK_T0(n_tput_hevc_epel_bi_v);
	...
	CLK_T1(n_tput_hevc_epel_bi_v);	
}

#ifdef EPEL_UNI_HV_SHORT_BW
                                       ...
                                       int height, intptr_t mx, intptr_t my, int width)
{
    ...
    uint8_t *tmp = tmp_array;
    int shift = 14 - BIT_DEPTH;
	//CLK_DEF(n_tput_hevc_epel_uni_hv)
	//CLK_T0(n_tput_hevc_epel_uni_hv)
#if BIT_DEPTH < 14
    int offset = 1 << (shift - 1);
#else
    int offset = 0;
    ...
    for (y = 0; y < height + EPEL_EXTRA; y++) {
        for (x = 0; x < width; x++)
            tmp[x] =av_clip_pixel( (EPEL_FILTER(src, 1) + 32) >> 6);
        src += srcstride;
        tmp += MAX_PB_SIZE;
    }
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++)
            dst[x] = av_clip_pixel(((EPEL_FILTER(tmp, MAX_PB_SIZE)) + offset) >> shift);
        tmp += MAX_PB_SIZE;
        dst += dststride;
    }
	//CLK_T1(n_tput_hevc_epel_uni_hv)
}
#else
static void FUNC(put_hevc_epel_uni_hv)(uint8_t *_dst, ptrdiff_t _dststride, uint8_t *_src, ptrdiff_t _srcstride,
                                       int height, intptr_t mx, intptr_t my, int width)
                                  ...
                                  int height, intptr_t mx, intptr_t my, int width)
{
    ...
    uint8_t tmp_array[(MAX_PB_SIZE + EPEL_EXTRA) * MAX_PB_SIZE];
    uint8_t *tmp = tmp_array;
	CLK_DEF(n_tput_hevc_epel_bi_hv)	
	CLK_T0(n_tput_hevc_epel_bi_hv)
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++)
            dst[x] = av_clip_pixel(((EPEL_FILTER(tmp, MAX_PB_SIZE)) + src2[x] + offset) >> shift);
        tmp  += MAX_PB_SIZE;
        dst  += dststride;
        src2 += MAX_PB_SIZE;
    }
	CLK_T1(n_tput_hevc_epel_bi_hv)	
}
#else
static void FUNC(put_hevc_epel_bi_hv)(uint8_t *_dst, ptrdiff_t _dststride, uint8_t *_src, ptrdiff_t _srcstride,
                                      int16_t *src2,
