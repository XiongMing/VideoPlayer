...
#ifdef HEVC_OPT_OPEN
   #define DBLK_SIMPLE_BS0
   #define DBLK_SIMPLE_BS1 
   #define DBLK_SIMPLE_BS2    
#else
#endif
...
static int set_sps(HEVCContext *s, const HEVCSPS *sps)
{
	...
	LOGI_point(init_emulated_edge_mc1, "The init edge mc point2");
...
static int hls_slice_header(HEVCContext *s)
{
    ...
    if (!sh->dependent_slice_segment_flag) {
		...
		//LOGI_point(frame_type,"frame %d", sh->slice_type);
...
#ifdef DBLK_SIMPLE_BS2
            if (s->HEVClc->cu.part_mode == PART_2Nx2N) {
                ff_hevc_deblocking_boundary_strengths_onepu(s, x0, y0, log2_trafo_size);
            }
			else {
                        ...
                        int block_w, int block_h, int luma_weight, int luma_offset)
{
    ...
    if (x_off < QPEL_EXTRA_BEFORE || y_off < QPEL_EXTRA_AFTER ||
		...
		//LOGI_point(emulated_edge_mc0, "The edge mc point0");
    ...
    if (!weight_flag)
    {
		 ...
		 //LOGI_point(point_nonweight_luma_mc_uni, "nonweight_luma_mc_uni point"); 		 
    }	
    else
    {   
		 ...
		 /*LOGI_point(point_weight_luma_mc_uni, "weight_luma_mc_uni_point %d %d %d",
...
#define POS(c_idx, x, y)                                                              \
    ...
    int x_pu, y_pu;
    int i, j;
	CLK_T0(thls_prediction_unit);
/*	
    if( (s->sh.slice_type == P_SLICE && s->pps->weighted_pred_flag) || (s->sh.slice_type == B_SLICE && s->pps->weighted_bipred_flag))
    {
	   LOGI_point("LOGI_ERR", "we do not suopport weight prediction");	
    }
*/
...
static int hls_coding_unit(HEVCContext *s, int x0, int y0, int log2_cb_size)
{
    ...
    int x, y, ret;
    CLK_DEF(thls_coding_unit);
    CLK_DEF(tcu_t0);	
    CLK_T0(thls_coding_unit);
    ...
    if (SAMPLE_CTB(s->skip_flag, x_cb, y_cb)) {
        ...
        if (!s->sh.disable_deblocking_filter_flag) 
#ifdef DBLK_SIMPLE_BS0			
			ff_hevc_deblocking_boundary_strengths_onepu(s, x0, y0, log2_cb_size);
#else
            ff_hevc_deblocking_boundary_strengths(s, x0, y0, log2_cb_size);
#endif
        ...
        if (lc->cu.pred_mode == MODE_INTRA) {
			...
			if ( lc->cu.part_mode != PART_2Nx2N ){
				LOGI_ERR("The prediction coding unit is not single");
			}
			*/
...
static int decode_nal_unit(HEVCContext *s, const uint8_t *nal, int length)
{
    ...
    int ctb_addr_ts, ret;
	CLK_DEF(tdecode_nal_unit);
	//CLK_DEF(tslice_header);
	//CLK_DEF(tslice_data);	
	CLK_T0(tdecode_nal_unit);
    ...
    case NAL_RASL_R:
	    ...
	    //CLK_T0(tslice_header);	
        ret = hls_slice_header(s);
	    //CLK_T1(tslice_header);		
        if (ret < 0)
            return ret;
...
static int decode_nal_units(HEVCContext *s, const uint8_t *buf, int length)
{
    ...
    CLK_DEF(tdecode_nal_units);
	CLK_T0(tdecode_nal_units);
    s->ref = NULL;
    s->last_eos = s->eos;
    s->eos = 0;
                             ...
                             AVPacket *avpkt)
{
    ...
    t0 = 1000.0*res.tv_sec + (double)res.tv_nsec/1e6;
    */

    HEVCContext *s = avctx->priv_data;
	CLK_DEF(thevc_decode_frame);
   	CLK_T0(thevc_decode_frame); 
	...
	/*
    clock_gettime(CLOCK_REALTIME, &res);
    t1 = 1000.0*res.tv_sec + (double)res.tv_nsec/1e6;		
	LOGI1("The delta time of hevc_decode_frame is %f\n", t1 - t0);
	*/
	CLK_T1(thevc_decode_frame);
    return avpkt->size;
}
