                                              ...
                                              const AVPacket *avpkt)
{
    ...
    AVPacket tmp = *avpkt;
	CLK_DEF(tavcodec_decode_video2);	
    CLK_T0(tavcodec_decode_video2);
	
    LOGI0("This is the entry of video decoding");
    ...
    *got_picture_ptr = 0;
    ...
    if ((avctx->codec->capabilities & CODEC_CAP_DELAY) || avpkt->size || (avctx->active_thread_type & FF_THREAD_FRAME)) {
		...
		{
			//LOGI_point(Thread_check , "This is the entry of other thread");
            ret = ff_thread_decode_frame(avctx, picture, got_picture_ptr,
                                         &tmp);
        }
        else {
        
			//LOGI_point(Thread_check , "This is the entry of main thread");	
